...
test.describe('Login entry', () => {
  ...
  test('shows secretary login option', async ({ page }) => {
    ...
    await expect(secretaryOption).toBeVisible({ timeout: 15000 });
  });
  ...
  test('shows role resolution error after forced signout', async ({ page }) => {
    ...
    await page.addInitScript((msg) => {
      window.localStorage.setItem('dh_role_resolution_error', msg);
    }, expectedMessage);

    // إعادة فتح الصفحة بعد تخزين الرسالة قبل mount حتى يقرأها التطبيق فوراً.
    await page.goto('/login?role-resolution-test=1', { waitUntil: 'domcontentloaded' });
  ...
  test('avoids redirect loop on patient host when role error is set', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      window.localStorage.setItem('__hostMode_override', 'patient');
      window.localStorage.setItem('dh_role_resolution_error', 'تعذَّر تحديد نوع حسابك...');
    });
