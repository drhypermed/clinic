    ...
    return (
        ...
        >
            <div className="p-6 text-center">
                ...
                <div className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-4 ${isDanger ? 'bg-danger-100 text-danger-600' : 'bg-brand-100 text-brand-600'}`}>
            ...
            <div className="bg-slate-50 p-4 flex gap-3 justify-center border-t border-slate-100">
                ...
                <button onClick={onConfirm} className={`px-5 py-2.5 rounded-xl font-bold text-white shadow-md transition-transform active:scale-95 ${isDanger ? 'bg-danger-600 hover:bg-danger-700' : 'bg-brand-600 hover:bg-brand-700'}`}>{confirmText}</button>
            </div>
        </ModalOverlay>
    );
};
