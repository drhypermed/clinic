...
export const SmartQuotaNoticeModal: React.FC<SmartQuotaNoticeModalProps> = ({ notice, onClose }) => {
  ...
  return createPortal(
    <div className={`fixed inset-0 z-[1000] flex items-center justify-center p-4 transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className={`relative w-full max-w-xl rounded-3xl bg-white shadow-2xl border border-success-100 p-5 sm:p-6 transform transition-all duration-300 ${
        isVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'
      }`}>
        <div className="text-center space-y-3" dir="rtl">
          {/* أيقونة الواتساب الترحيبية */}
          <div className="mx-auto w-14 h-14 rounded-2xl bg-success-100 text-success-700 flex items-center justify-center animate-in fade-in zoom-in duration-500">
          ...
          <div className="flex justify-center gap-3">
            ...
            {notice.whatsappUrl && (
              <a
                ...
                className="inline-flex items-center px-4 py-2 rounded-xl bg-success-600 hover:bg-success-700 text-white font-black transition-colors"
              >
                تواصل واتساب
              </a>
            )}
