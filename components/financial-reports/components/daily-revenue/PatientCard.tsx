...
export const PatientCard: React.FC<PatientCardProps> = ({
    ...
    <div className={`rounded-xl border p-2.5 ${
        isInsurance ? 'bg-blue-50/50 border-blue-100' : 'bg-white border-slate-100'
    }`}>
        <div className="flex items-start justify-between gap-1">
            <p className={`text-[10px] font-bold truncate flex-1 min-w-0 ${
                isInsurance ? 'text-blue-600' : 'text-slate-500'
            }`}>
            ...
            {(onEdit || onDelete) && (
                <div className="flex items-center gap-1 shrink-0">
                    {onEdit && (
                        <button onClick={onEdit} className="w-5 h-5 rounded-md bg-blue-50 hover:bg-blue-100 flex items-center justify-center transition-colors" title="تعديل">
                    ...
                    {onDelete && (
                        <button onClick={onDelete} className="w-5 h-5 rounded-md bg-rose-50 hover:bg-rose-100 flex items-center justify-center transition-colors" title="حذف">
...
export const RevenueSection: React.FC<RevenueSectionProps> = ({ emoji, title, children }) => (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100">
        <div className="flex items-center gap-2 px-3 py-2.5 bg-gradient-to-r from-blue-700 to-blue-600">
