...
export const GINA_2025_ADULT_MEDICATION_TOPICS: GuidelineTopic[] = [
  {
    ...
    points: {
      ...
      ar: [
        'تأكد من التشخيص واستخدام البخاخة بشكل صحيح قبل رفع الجرعة (Step-up).',
        'إذا زادت الأعراض، يُفضل رفع الجرعة لفترة قصيرة (1-2 أسبوع) لمواجهة عدوى فيروسية أو التعرض لمسببات الحساسية.',
        'لا تُقلل الجرعة (Step-down) إلا إذا تم السيطرة على الربو واستقراره لمدة 2-3 أشهر.',
        'يجب أن يكون التقليل تدريجياً (خفض جرعة الكورتيزون المستنشق بنسبة 25-50% في المرة الواحدة).',
        'تجنب إيقاف الكورتيزون الوقائي (ICS) تماماً للبالغين، حيث يؤدي ذلك لزيادة خطر الانتكاسات الشديدة بشكل كبير.',
  ...
  },
  {
    ...
    points: {
      ...
      ar: [
        'المسار الأول هو (المفضل) لأنه يقلل خطر الانتكاسات الشديدة بنسبة 65% مقارنة بالمسار الثاني، باستخدام بخاخة واحدة لجميع الدرجات.',
        'الدرجة 1 و 2: بخاخة ICS-formoterol بجرعة منخفضة تُؤخذ (عند اللزوم فقط). هذا يقلل زيارات الطوارئ بنسبة الثلثين مقارنة بالفينتولين.',
        ...
        'الحدود القصوى: بوديزونيد-فورموتيرول (Budesonide-Formoterol 160/4.5 mcg كالسيمبيكورت): بحد أقصى 12 بخة/يوم. بيكلوميثازون-فورموتيرول (Beclometasone-Formoterol 100/6 mcg كالفوستير): بحد أقصى 12 بخة/يوم.',
