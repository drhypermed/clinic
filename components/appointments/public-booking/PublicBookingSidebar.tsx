...
const navItems: {
...
}[] = [
  {
    ...
    activeGradient: 'from-brand-600 to-success-600',
    iconBgActive: 'bg-brand-500',
  },
  {
    ...
    activeGradient: 'from-slate-600 to-slate-600',
    iconBgActive: 'bg-slate-500',
    badgeKey: 'today',
  },
  {
    ...
    activeGradient: 'from-warning-500 to-warning-500',
    iconBgActive: 'bg-warning-500',
    badgeKey: 'upcoming',
  },
  {
    ...
    activeGradient: 'from-success-600 to-success-600',
    iconBgActive: 'bg-success-500',
  },
  {
    id: 'publicForm',
    label: 'فورم الجمهور',
    ...
    activeGradient: 'from-warning-500 to-danger-500',
    iconBgActive: 'bg-warning-500',
  },
];
...
export const PublicBookingSidebar: React.FC<PublicBookingSidebarProps> = ({
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="border-b border-slate-100 flex flex-col items-center shrink-0 p-4">
        <div className="relative mb-3">
          <div className="mx-auto rounded-full p-[3px] bg-gradient-to-tr from-slate-300 via-slate-200 to-slate-100 w-20 h-20 shadow-lg">
            <div className="w-full h-full rounded-full overflow-hidden bg-gradient-to-br from-brand-500 to-brand-600 flex items-center justify-center text-white font-black text-xl border-2 border-white">
              {secretaryAvatarText}
            </div>
          </div>
        </div>
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-brand-600 to-brand-700 text-white border border-brand-500/60 hover:brightness-105 transition-all text-sm font-bold"
        >
      ...
      <nav className="flex-1 flex flex-col p-3 space-y-1.5 overflow-y-auto">
            ...
            >
              ...
              {badge !== undefined && (
                <span className={`absolute left-3 top-1/2 -translate-y-1/2 min-w-[22px] h-[22px] px-1.5 flex items-center justify-center rounded-full text-[11px] font-black ${
                  isActive ? 'bg-white/25 text-white' : 'bg-slate-500 text-white'
                }`}>
                  {badge > 99 ? '99+' : badge}
                </span>
              )}
        ...
        <button
          onClick={onLogout}
          className="group w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-200 text-danger-500 hover:bg-danger-50 hover:text-danger-600"
        >
          <div className="w-9 h-9 shrink-0 rounded-lg flex items-center justify-center bg-danger-50 group-hover:bg-danger-100 transition-all">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <>
      {/* Mobile Header */}
      <div className={`md:hidden fixed top-0 right-0 left-0 bg-white/95 backdrop-blur border-b border-brand-200 shadow-sm z-[999] transition-transform duration-200 ease-out will-change-transform ${mobileHeaderHidden ? '-translate-y-full' : 'translate-y-0'}`}>
        <div className="flex items-center justify-between px-3 py-2.5">
          ...
          <button onClick={onOpenProfile} className="w-9 h-9 rounded-full bg-gradient-to-br from-brand-500 to-brand-600 text-white flex items-center justify-center font-black text-[10px] border-2 border-white shadow-md shrink-0">
            {secretaryAvatarText}
          </button>
        </div>
      </div>
