  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2.5 px-1 pb-1 flex-wrap">
        ...
        <h3 className="text-sm font-black text-blue-800">مواعيد اليوم</h3>
        ...
        {sortedTodayAppointments.length > 0 && (
          <span className="mr-auto text-[11px] font-black bg-blue-100 text-blue-700 rounded-full px-2.5 py-0.5 border border-blue-200">
              ...
              <div key={apt.id} className="dh-patient-shell rounded-2xl border overflow-hidden">
                <div className="p-3 space-y-2 text-right">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="rounded-full border border-blue-300 bg-blue-100 px-2 py-0.5 text-[10px] font-black text-blue-800">#{idx + 1}</span>
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 text-[11px] font-black">
                    ...
                    <span className={`rounded-full border px-2 py-0.5 ${isConsultation ? 'bg-purple-100 text-purple-700 border-purple-200' : 'bg-teal-100 text-teal-700 border-teal-200'}`}>
                  ...
                  <div className="flex items-center gap-2 flex-wrap pt-1">
                    {isApproved ? (
                      <span className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-emerald-100 text-emerald-700 font-bold text-[11px] border border-emerald-200">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg>
                        تم الدخول
                      </span>
                    ) : isPending ? (
                      <span className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-amber-100 text-amber-700 font-bold text-[11px] border border-amber-200">
                    ...
                    <button type="button" onClick={() => onEditAppointment(apt)} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs shadow-md transition-all">
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                      تعديل
                    </button>
                    <button type="button" onClick={() => onRemoveTodayAppointment(apt.id)} className="p-1.5 rounded-xl border border-slate-200 text-slate-500 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-all" title="حذف">
