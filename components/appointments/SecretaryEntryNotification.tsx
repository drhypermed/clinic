...
export const SecretaryEntryNotification: React.FC<SecretaryEntryNotificationProps> = ({
  ...
  <div
    ...
    dir="rtl"
  >
    <div className="bg-white rounded-2xl shadow-xl border-2 border-slate-400 p-3 sm:p-4 w-full">
      <div className="grid grid-cols-[auto_1fr] items-start gap-2.5 sm:gap-3 min-w-0">
        {/* أيقونة الجرس للتنبيه */}
        <span className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-slate-100 flex items-center justify-center shrink-0">
      ...
      <div className="grid grid-cols-2 gap-2 mt-3">
        <button onClick={onApprove} className="inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl bg-success-600 hover:bg-success-700 text-white font-bold text-sm w-full shadow-lg transition-transform active:scale-95">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg>
          دخول
        </button>
        <button onClick={onReject} className="inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl bg-warning-500 hover:bg-warning-600 text-white font-bold text-sm w-full shadow-lg transition-transform active:scale-95">
