...
export const SecretaryVitalsPills: React.FC<SecretaryVitalsPillsProps> = ({
  ...
  return (
    <div className={`${className}`.trim()}>
      {title && (
        <p className={`font-black text-orange-800 mb-1.5 ${compact ? 'text-[10px]' : 'text-[11px]'}`}>{title}</p>
      )}
      <div className="flex flex-wrap gap-1.5">
        {entries.map((entry) => (
          <span
            key={entry.key}
            className={`inline-flex items-center gap-1 rounded-full border border-orange-200 bg-orange-50 font-bold text-orange-800 ${
              compact ? 'px-2 py-0.5 text-[10px]' : 'px-2.5 py-1 text-[11px]'
            }`}
          >
            <span className="font-black">{entry.shortLabel || entry.label}</span>
            <span className="text-orange-600">{entry.value}{entry.unit ? ` ${entry.unit}` : ''}</span>
          </span>
        ))}
      </div>
    </div>
