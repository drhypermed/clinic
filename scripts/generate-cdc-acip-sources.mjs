...
const sourceDir = path.resolve('guidelines-sources/_structured/full-text/CDC_ACIP');
const outputFile = path.resolve('components/guidelines/data/cdcAcip2026/sources.ts');
...
for (const filepath of walkJsonFiles(sourceDir)) {
  ...
  const topic = parts[1] || 'ACIP Vaccine Recommendations';
  const title = path.posix.basename(sourcePath, path.posix.extname(sourcePath));
  const fileId = `cdc-acip-${slugify(topic)}-${slugify(title)}`.slice(0, 100);

  sources.push({
    id: fileId,
    folderTitle: topic,
    folderTopicId: `cdc-acip-folder-${slugify(topic)}`.slice(0, 90),
    fileTopicId: `cdc-acip-file-${fileId}`.slice(0, 120),
    title,
    fileType: 'Full guideline',
    pageCount: data.pageCount,
    textChars: data.textChars,
    chunkCount: Array.isArray(data.chunks) ? data.chunks.length : 0,
    citation: `Centers for Disease Control and Prevention. ${title}.`,
...
out += 'export const CDC_ACIP_SOURCES: GuidelineSource[] = ';
