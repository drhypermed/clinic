...
const sourceDir = path.resolve('guidelines-sources/_structured/full-text/Endocrine');
const outputFile = path.resolve('components/guidelines/data/endocrine2026/sources.ts');
...
for (const filepath of jsonFiles) {
  ...
  const category = parts.length > 2 ? parts[1] : 'Endocrine Society Guidelines';
  const filename = path.posix.basename(sourcePath);
  const title = path.posix.basename(filename, path.posix.extname(filename));
  const fileId = `endocrine-${slugify(category)}-${slugify(title)}`.slice(0, 100);
  const folderId = `endocrine-folder-${slugify(category)}`.slice(0, 90);

  sources.push({
    id: fileId,
    folderTitle: category,
    folderTopicId: folderId,
    fileTopicId: `endocrine-file-${fileId}`.slice(0, 120),
    title,
    fileType: 'Full guideline',
    pageCount: data.pageCount,
    textChars: data.textChars,
    chunkCount: Array.isArray(data.chunks) ? data.chunks.length : 0,
    citation: `Endocrine Society. ${title}.`,
...
out += 'export const ENDOCRINE_SOURCES: GuidelineSource[] = ';
