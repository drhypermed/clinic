/**
 * Extract full text from Endocrine Society PDFs using pdfjs-dist for 100% accuracy.
 * Run: node scripts/extract-endocrine-text.mjs
 */
...
const sourceDir = path.resolve('guidelines-sources/Endocrine');
const outputDir = path.resolve('guidelines-sources/Endocrine/extracted');
...
const main = async () => {
  ...
  for (const file of files) {
    ...
    // Ignore empty/redirect files that are too small
    if (fs.statSync(pdfPath).size < 20 * 1024) {
