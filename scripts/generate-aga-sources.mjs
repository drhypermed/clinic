...
const sourceDir = path.resolve('guidelines-sources/_structured/full-text/AGA');
const outputFile = path.resolve('components/guidelines/data/aga2026/sources.ts');
...
for (const filepath of walkJsonFiles(sourceDir)) {
  ...
  const category = parts.length > 2 ? parts[1] : 'AGA Clinical Guidance';
  const filename = path.posix.basename(sourcePath);
  const title = path.posix.basename(filename, path.posix.extname(filename));
  const fileId = `aga-${slugify(category)}-${slugify(title)}`.slice(0, 100);
  const folderId = `aga-folder-${slugify(category)}`.slice(0, 90);

  sources.push({
    id: fileId,
    folderTitle: category,
    folderTopicId: folderId,
    fileTopicId: `aga-file-${fileId}`.slice(0, 120),
    title,
    fileType: 'Full guideline',
    pageCount: data.pageCount,
    textChars: data.textChars,
    chunkCount: Array.isArray(data.chunks) ? data.chunks.length : 0,
    citation: `American Gastroenterological Association. ${title}.`,
...
out += 'export const AGA_SOURCES: GuidelineSource[] = ';
